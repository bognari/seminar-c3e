...
\begin{abstract}
%\boldmath
In diesem Paper werden die 10 von Xindong Wu \cite{wu2008top} 
beschriebenen Data-Mining-Algorithmen eingeordnet sowie die wichtigsten vorgestellt. Zu diesem Zweck wird auf die Kategorie der Cluster-,
...
\IEEEPARstart{D}{ata Mining} wird in der Forschung, Vermarktung, Medizin, (Wetter)vorhersagen, Betrugsaufklärung usw.~eingesetzt. Der Begriff \enquote{Data Mining} wird allerdings im Deutschen fälschlicherweise für den gesamten Prozess der \emph{Knowledge Discovery in Databases} (KDD) verwendet. Wie in Abbildung \ref{kdd} ersichtlich, stellt Data Mining jedoch lediglich einen Schritt im Prozess der KDD dar. Für KDD werden zuerst die Daten gesammelt, aufbereitet und homogenisiert und dann mittels eines Data-Mining-Algorithmus verarbeitet, sodass Muster entstehen, die schließlich zum Ziel des \emph{Wissens} interpretiert werden. Im Folgenden seien die wichtigsten Kategorien dieser Data-Mining-Algorithmen erläutert.
 
\subsection{Clustering-Algorithmen}

\IEEEPARstart{D}{ie Kategorie der Clustering-Algorithmen} beinhaltet Algorithmen, die Daten unbekannten Klassen, den so genannten \emph{Clustern} (Abschnitt \ref{cluster}), zuordnen. Während des Clustering wird also eine Funktion gesucht, die Objekte auf Gruppen abbildet. Diese Gruppierung wird mithilfe
...
\begin{figure}[!t]
\centering
\subfigure[Phase 1 -- 3]{\scalebox{0.5}{\input{../beamer/img/kmeans1.tikz}}\label{p1-3 k-means}}
\subfigure[Phase 4]{\scalebox{0.5}{\input{../beamer/img/kmeans3.tikz}}\label{p4 k-means}}
\subfigure[Phase 5]{\scalebox{0.5}{\input{../beamer/img/kmeans2.tikz}}\label{p5 k-means}}
\subfigure[Phase 6]{\scalebox{0.5}{\input{../beamer/img/kmeans4.tikz}}\label{p6 k-means}}
\caption{k-means}
\label{k-means 1-6}
\end{figure}
...
\begin{figure}[!t]
\centering
\subfigure[Gesucht]{\scalebox{0.42}{\input{../beamer/img/svm1.tikz}}\label{hyperplane gesucht}}
\subfigure[Gefunden]{\scalebox{0.42}{\input{../beamer/img/svm2.tikz}}\label{hyperplane gefunden}}
\subfigure[Einordnung]{\scalebox{0.49}{\input{../beamer/img/svm2b.tikz}}\label{einordnung}}
\subfigure[Ungültige Daten]{\scalebox{0.49}{\input{../beamer/img/svm3.tikz}}\label{ungueltig}}
\caption{SVM}
\label{svm_pic}
\end{figure}
...
\subsection{Assoziation-Algorithmen}
 \IEEEPARstart{D}{ie} meisten online Einkäufer ist der Satz \enquote{
...
\IEEEPARstart{D}{as} HACE Theorem wird im behandelt die dauerhafte Generierung unterschiedlichster und im Extremfall ungenauer bis falscher Daten 
...
\label{bigdata}
\end{figure}
\IEEEPARstart{D}{urch} das HACE Theorem entstehen für das Data Mining viele Schwierigkeiten. In der Abbildung \ref{bigdata} 

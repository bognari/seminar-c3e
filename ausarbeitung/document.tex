...
von Xindong Wu vorgestellt sowie eingeordnet. Anschließend wird auf Big Data sowie die Verbindung zwischen 
...
Attribut ist entweder ein Nummerisches oder Kategorisches-Attribut und besitzt für sich selbst 
spezielle Distanzfunktionen. Die Bedingungen \ref{dist1} -- \ref{dist3} müssen für jede 
Distanzfunktion gelten. Wenn die Bedingung \ref{dist4} gilt, handelt es sich um eine Metrik.
\begin{align}
\dist(o_1, o_2) &= d \in R^{n\geq 0} \label{dist1}\\
\dist(o_1, o_2) &= 0 \Leftrightarrow o_1 = o_2 \label{dist2}\\
\dist(o_1, o_2) &= \dist(o_2, o_1) \mbox{ (Symmetrie)} \label{dist3}\\
\dist(o_1, o_3) &\leq \dist(o_1, o_2) + \dist(o_2, o_3) \label{dist4}
\end{align} 

\IEEEPARstart{F}{ür} Nummerische-Attribute existieren die folgenden Distanzfunktionen:
\begin{align}
\intertext{Euklidische-Distanz:\vspace{-0.8em}}
\dist(x,y) &= \sqrt{(x_1 - y_1)^2 + \ldots + (x_n - y_n)^2}\\
\intertext{Manhattan-Distanz:\vspace{-0.8em}}
\dist(x,y) &= |x_1 - y_1| + \ldots + |x_n - y_n|\\
\intertext{Maximum-Metrik:\vspace{-0.8em}} 
\dist(x,y) &= \max\big(|x_1 - y_1| + \ldots + |x_n - y_n|\big)\\
\intertext{Alg. $L_p$-Metrik:\vspace{-0.8em}}
\dist(x,y) &= \sqrt[p]{\sum\limits_{i=1}^{d}{(x_i-y_i)^p}}
\end{align}

\IEEEPARstart{F}{ür} Kategorische-Attribute existiert die die Summe 
der Unterschiede als Distanzfunktion, die wie folgt definiert ist:
 \begin{align}
 \dist(x,y) &= \sum\limits_{i=1}^{a}\delta(x_i, y_i)\\
...
\end{figure} 
 ...
 Aus diesen Trainingsdaten wird der, in Listing \ref{dt} als Bedingung formulierter, Entscheidungsbaum
...
Der mittlere Hyperplanekadidaten ist die richtige Hyperplane und ist in Abbildung \ref{hyperplane gefunden} zu sehen. 
...
Hyperplane sie sich befinden den beiden Klassen zugeordnet. Dieses ist in Abbildung \ref{einordnung} gezeigt. 
In Abbildung \ref{ungueltig} sind ungültige Trainigsdaten dargestellt. Bei diesen Daten kann keine gültige Hyperplane 
